...
with st.sidebar:
    ...
    nav = st.radio(
        "Navigatie", ["Dashboard", "Inschrijven", "About"], label_visibility="collapsed"
    )
    ...
    with st.form("postal_code_form_sidebar"):  # Unieke key
        st.text_input(
            "Voer een postcode in (bijv. 8937 AC):", key="sidebar_postcode_input"
        )
...
def page_home():
    ...
    st.markdown(
        "<h1><font color=red>HEAT</font> stress APP</h1>", unsafe_allow_html=True
    )
...
def page_register():
    ...
    st.markdown(
        "Wil je ook een maatwerk alert op je telefoon ontvangen. Vul dan het onderstaand formulier in."
    )
    ...
    with st.form("register_form"):
        ...
        ubn = st.text_input(
            "UBN NUmmer", help="Uniek Bedrijfs Nummer (indien van toepassing)"
        )
        ...
        birthdate = st.text_input(
            "Geboortedatum", help="Voor leeftijdsgebonden advies (DD-MM-JJJJ)"
        )
        bankrekening = st.text_input("Bankrekening", help="Voor incasso (IBAN)")
        geslacht = st.selectbox("Geslacht", ["Man", "Vrouw"])
        soortBoerderij = st.selectbox(
            "Soort Boerderij", ["Melkveebedrijf", "Vleesveebedrijf", "Overig"]
        )
        ...
        if submitted:
            ...
            if not name or not address or not city or not phone:
                ...
                st.success(
                    f"Bedankt voor je inschrijving, {name}! Je ontvangt binnenkort een maatwerk alert."
                )
                st.info(
                    "Let op: Dit formulier slaat momenteel geen gegevens op en verstuurt geen echte alerts."
                )
